...
export default function JoinScreen({ setScreen, screens, roomId }) {
  ...
  const startLocalStream = async () => {
    ...
    console.log("Come via join screen : ",constraints);
    const newStream = await mediaDevices.getUserMedia(constraints);
    console.log("Come via join screen  newStream newStream: ",newStream);

    setLocalStream(newStream);
  };

  const joinCall = async id => {
    console.log("Join screen iddd==>",id);
    const roomRef = await db.collection('rooms').doc(id);
    const roomSnapshot = await roomRef.get();
    console.log("Join screen roomRef==>",roomRef);
    console.log("Join screen second roomRef==>",roomSnapshot);
    if (!roomSnapshot.exists) return
    const localPC = new RTCPeerConnection(configuration);
    localPC.addIceCandidate(localStream);
